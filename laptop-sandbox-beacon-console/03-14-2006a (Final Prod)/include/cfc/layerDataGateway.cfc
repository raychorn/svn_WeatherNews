...
<cffunction name="buildFlights" access="remote" returntype="array" output="false">	
		<cfscript>		
		var flightArray = arrayNew(1);	//this is okay
		var flightCounter = 10; //The flight counter is going to be a numrecords from the query obj
		for(i=1;i LT flightCounter; i=i+1) {		
			///This area would be a for loop populated by the database....or a query object with some additional work.				
			flightStruct = structNew();
			histPathArray = arrayNew(1);
			futurePathArray = arrayNew(1);
			//Flight ID
			flightCounterString = toString(i);			
			flightStruct.FL_ID = "FL1205" & flightCounterString;  //this should be the actual flight id

			//Relative Depth
			flightStruct.DepthBase = 2010+(i*10);  //This is ok
			
			//Arrival and Departure Details  ---  from the database
			flightStruct.DestPointLat = 51.47+i;
			flightStruct.DestPointLon = -0.451;
			flightStruct.OriginPointLat = 40.63+i;
			flightStruct.OriginPointLon = -73.77;
			
			//Current position  --- from the database
			flightStruct.CurrPointLat = 59.62+i;
			flightStruct.CurrPointLon = -36.00+(i*(flightCounter/10));
			flightStruct.Heading = 250;
			
			//Historical Points  --- from the database
				flightStruct.histPathArray = arrayNew(1);
				flightStruct.histPathArray[1] = structNew();
				flightStruct.histPathArray[1].HistoricalPointLat = 43.35+i;
				flightStruct.histPathArray[1].HistoricalPointLon = -69.04-i;
				flightStruct.histPathArray[1].HistoricalPointAlt = 380;
				flightStruct.histPathArray[1].HistoricalPointHeading = 375;
				flightStruct.histPathArray[1].HistoricalPointSpeed = 225;
				flightStruct.histPathArray[2] = structNew();			
				flightStruct.histPathArray[2].HistoricalPointLat = 54.88+i;
				flightStruct.histPathArray[2].HistoricalPointLon = -46.08;
				flightStruct.histPathArray[2].HistoricalPointAlt = 380;
				flightStruct.histPathArray[2].HistoricalPointHeading = 375;
				flightStruct.histPathArray[2].HistoricalPointSpeed = 225;
				flightStruct.histPathArray[3] = structNew();			
				flightStruct.histPathArray[3].HistoricalPointLat = 56.88+i;
				flightStruct.histPathArray[3].HistoricalPointLon = -40.08-i;
				flightStruct.histPathArray[3].HistoricalPointAlt = 380;
				flightStruct.histPathArray[3].HistoricalPointHeading = 375;
				flightStruct.histPathArray[3].HistoricalPointSpeed = 225;
				
			//Future Points  --- from the database
				flightStruct.futurePathArray = arrayNew(1);
				flightStruct.futurePathArray[1] = structNew();
				flightStruct.futurePathArray[1].FuturePointLat = 63.19+i;
				flightStruct.futurePathArray[1].FuturePointLon = -26.86-i;
				flightStruct.futurePathArray[1].FuturePointAlt = 380;
				flightStruct.futurePathArray[1].FuturePointHeading = 375;
				flightStruct.futurePathArray[1].FuturePointSpeed = 225;
				flightStruct.futurePathArray[2] = structNew();			
				flightStruct.futurePathArray[2].FuturePointLat = 63.71+i;
				flightStruct.futurePathArray[2].FuturePointLon = -19.74;
				flightStruct.futurePathArray[2].FuturePointAlt = 380;
				flightStruct.futurePathArray[2].FuturePointHeading = 375;
				flightStruct.futurePathArray[2].FuturePointSpeed = 225;
				flightStruct.futurePathArray[3] = structNew();			
				flightStruct.futurePathArray[3].FuturePointLat = 58.93+i;
				flightStruct.futurePathArray[3].FuturePointLon = -9.10-i;
				flightStruct.futurePathArray[3].FuturePointAlt = 380;
				flightStruct.futurePathArray[3].FuturePointHeading = 375;
				flightStruct.futurePathArray[3].FuturePointSpeed = 225;			
			
			flightArray[i] = flightStruct;  //this  is ok
		...
		<cfreturn flightArray />
	</cffunction>
	
	
<cffunction name="buildSevereWeather" access="remote" returntype="struct" output="false">	
		<!--- <cfset timeNow = #TimeFormat(now(),"HH:MM")#>
		<cfset dateNow = #dateFormat(Now(),"yyyy-mm-dd")#> --->
		
		<cfscript>		
		severeWxStruct = structNew();
		bounding_box = Arguments[1].bBox;
		struct_coordinates = deriveCoordinates(bounding_box);		
		
		/*
		SELECT subType, wboxID, rawMsg, pointList
				FROM avnfcstdb..watch_box
				WHERE wboxType=‘WCP’
				AND minLat < #struct_coordinates.x3#
				AND maxLat > #struct_coordinates.x2#
				AND minLon < #struct_coordinates.y3#
				AND maxLon > #struct_coordinates.y1#
		*/
		</cfscript>
		<cfquery name="select_severewx" datasource="#request.AVNOPS_DS#">
				SELECT subtype, wboxID, rawMsg, pointList, wboxType, startTime, endTime
				FROM avnfcstdb..watch_box
				WHERE (wboxType='WATCH' OR wboxType='WCP') AND endTime > getdate() AND startTime <= getdate() ORDER BY subtype DESC
		</cfquery>
		<cfscript>	
		severeWxStruct.QUERYRESULT = select_severewx;
		</cfscript>
	<cfreturn severeWxStruct />
</cffunction>
 ...
 --->
	<cffunction name="objToFlashAnimation" access="remote" returntype="struct" output="false">
		...
		<cfloop list="#normalLyr#" index="nmlyr">
			<cfif nmlyr EQ 0 OR listFindNoCase(backgroundLayers,nmlyr) EQ 0>
				<!--- Do Nothing --->
			<cfelse>
		...
		<cfif Right(backgroundLayers,1) EQ ",">
		...
		<!---
		<cfsavecontent variable="anistruct">
			<cfoutput>CONUSIR_6,Borders,Coastlines,US State Borders,Counties#backgroundLayers#</cfoutput>
		</cfsavecontent>
		<cffile action="write" file="#ExpandPath('../include/debug/animationInput.html')#" output="#anistruct#" nameconflict="overwrite" />
		--->
		...
		<cfset arguments.serverURL = "http://demis-dev.wni.com/" />
