...
AJAXEngine.prototype = {
	id : -1,
	errors : [],
	iframeSrc : '/include/ajax/ajaxDriver.cfm',
	sent : null,
	received : null,
	counter : 0,
	multithreaded : true,
...
menuEditorObj.defaultGUI = function(_menuEditorObj, _site_menu_text_color, _outerTableWidth) {
	...
	_html += '<tr><td align="left" valign="top"><div><table width="*" cellpadding="-1" cellspacing="-1"><tr>';
	_html += '<td width="*" align="right" valign="top"><table width="100%" cellpadding="-1" cellspacing="-1"><tr><td>';
	_html += '<table cellpadding="-1" cellspacing="-1"><tr><td><div id="btn_menuItemEditor" style="display: inline;">';
	...
	_html += '</div></td></tr><tr><td>&nbsp;</td></tr><tr><td><div id="div_menuItemAdder" style="display: inline;">';
	...
	_html += '</div></td></tr><tr><td>&nbsp;</td></tr><tr><td><div id="div_menuAdder" style="display: inline;">';
...
menuEditorObj.prototype = {
	...
	onOpenAddSubMenu : function(brObj) {
		return brObj; 
	},
	...
	_processMenuSubMenuAdditor : function(brObj) {
		...
		this.onOpenAddSubMenu(this.brObj());
...
ClientMenuObj.prototype = {
	...
	openClientSubMenuByUUID : function(aUUID, uuids) {
		...
		var bool_useRadioButtonsFromDb = false;
		var uAR = this.cuMenuDict.getValueFor(this.aSUBMENUUUID);
		if (typeof uAR == const_object_symbol) {
			uAR = uAR.pop();
		}
		tDict = DictionaryObj.getInstance(uAR);
		var _RB_OPTIONS = tDict.getValueFor('RB_OPTIONS');
		...
		if (!!dObj) {
			if (dObj.innerHTML.length > 0) {
				...
				bool_useRadioButtons = ( ((!!_RB_OPTIONS) ? (_RB_OPTIONS.trim().length > 0) : false) || this.onL2DecideToUseRadioButtonOrCheckBoxes(aUUID) );
				...
				_html = '<table width="' + (156 - 5) + '" cellpadding="-1" cellspacing="-1"' + ((this.bool_adjustMarginLeft) ? ' style="margin-left: -20px;"' : '') + '>';
				for (i = 0; i < iAR.length; i++) {
					_html += '<tr class="sideMenuBG3">';
					tDict = DictionaryObj.getInstance(iAR[i]);
					mUUID = tDict.getValueFor('MENUUUID');
					mLEGACYID = tDict.getValueFor('LEGACYID');
					...
					if (hasChildren) {
						...
						for (j = 0; j < oChildren.length; j++) {
							...
							mLEGACYID3 = tDict3.getValueFor('LEGACYID');
							var isChecked = 0;
							var arrCurrLyr = currentLayerList.split(",");
							for(zq=0;zq<arrCurrLyr.length;zq++){
								if(arrCurrLyr[zq] == mLEGACYID3) {
									isChecked = 1;
								}
							}
							_html += '<input type="' + ((bool_useRadioButtons == false) ? 'Checkbox' : 'Radio') + '" class="sideMenuCB" id="cb_menuItem_' + mUUID3 + '"' + ((bool_useRadioButtons == false) ? '' : 'name=rb_menuItem_' + this.aSUBMENUUUID) + ' onclick="' + ((bool_useRadioButtons == false) ? 'newMapLayer(' + getFlashID() + ', ' + mLEGACYID3 + ' );ClientMenuObj.onCheckBoxClicked(ClientMenuObj.instances[' + this.id + '], this.id, this.checked); return true;' : 'removeAndAddLayer(\'' + getFlashID() + '\',' + mLEGACYID3 + ',\'18,16,10,11,12\');turnOffAnimation();') + '" ' + ((isChecked == 1) ? 'checked' : ' ') + '>&nbsp;' + '<span class="menuClass">' + tDict3.getValueFor('PROMPT') + '</span>';
							_html += '</td>';
							_html += '</tr>';
						...
						var arrCurrLyr = currentLayerList.split(",");
						for(zq=0;zq<arrCurrLyr.length;zq++){
							if(arrCurrLyr[zq] == mLEGACYID) {
								isChecked = 1;
							}
						}
						if (bool_useLinks == false) {
							_html += ((mUUID.trim().length > 0) ? '<input type="' + ((bool_useRadioButtons == false) ? 'Checkbox' : 'Radio') + '" class="sideMenuCB" id="cb_menuItem_' + mUUID + '"' + ((bool_useRadioButtons == false) ? '' : 'name=rb_menuItem_' + this.aSUBMENUUUID) + ' onclick="' + ((bool_useRadioButtons == false) ? 'newMapLayer(' + getFlashID() + ', ' + mLEGACYID + ' );ClientMenuObj.onCheckBoxClicked(ClientMenuObj.instances[' + this.id + '], this.id, this.checked); return true;' : 'removeAndAddLayer(\'' + getFlashID() + '\',' + mLEGACYID + ',\'18,16,10,11,12\');turnOffAnimation();') + '" ' + ((isChecked == 1) ? 'checked' : ' ') + '>&nbsp;' : '');
						}
		...
		if (!!dObj) {
			...
			if (typeof iAR == const_object_symbol) {
				...
				for (i = 0; i < iAR.length; i++) {
					...
					tDict = DictionaryObj.getInstance(iAR[i]);
					mUUID = tDict.getValueFor('MENUUUID');
					_html += '<td><div class="menuHeader" id="div_menu_' + mUUID + '" onclick="menuArrowFlipper(\'menuArrow_' + mUUID + '\');ClientMenuObj.openClientSubMenuByUUID(ClientMenuObj.instances[' + this.id + '],\'' + mUUID + '\', \'' + siblingsUUIDs + '\'); return false;"><span class="menuClass">' + '<img id="menuArrow_' + mUUID + '" src="/images/MenuArrowOff.gif" />' + tDict.getValueFor('PROMPT') + '</span></div><div class="divSubmenu" id="div_subMenu_' + mUUID + '" style="display: none;"></div></div></td>';
					DictionaryObj.removeInstance(tDict.id);
					_html += '</tr>';
				}
			} else {
