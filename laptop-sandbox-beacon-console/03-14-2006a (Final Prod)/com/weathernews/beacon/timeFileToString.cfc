	...
	<cffunction name="pullTime" access="package" returntype="string">
		<cfargument name="strFileName" type="string" required="yes" />
			<cfswitch expression="#ListLast(ListGetAt(arguments.strFileName,2,':'),'.')#">
				...
				<cfcase value="xml">
					...
					<cfloop list="#ListGetAt(arguments.strFileName,2,':')#" delimiters="," index="fileName">
						<cffile action="read" 
						...
						<cfif CompareNoCase(variables.issuedTime[1].XmlText,variables.validTime[1].XmlText) EQ 0>
							<cfset variables.fullString = variables.fullString & " " & variables.issuedTime[1].XmlText />
						<cfelse>
							<cfset variables.fullString = variables.fullString & " " & variables.issuedTime[1].XmlText & " " & variables.validTime[1].XmlText />
						</cfif>
		...
		<cfreturn variables.fullString />
				...
				<cfcase value="txt">
					...
					<cfloop list="#ListGetAt(arguments.strFileName,2,':')#" delimiters="," index="fileName">
						...
						<cfif fileName EQ ListFirst(fileName,",")>
							<cfset variables.fullString = variables.fullString & " " & variables.title & " " & variables.txtYear & "/" & variables.txtMonth & "/" & variables.txtDay & "&nbsp;" & variables.txtHour & ":" & variables.txtMinutes />
						<cfelse>
							<cfset variables.fullString = variables.fullString & " " & " " & variables.txtYear & "/" & variables.txtMonth & "/" & variables.txtDay & "&nbsp;" & variables.txtHour & ":" & variables.txtMinutes  />
						</cfif>
						...
						<cfif Len(variables.fullString) GT 120>
							<cfset variables.fullString = "<br />" & variables.fullString & ListGetAt(arguments.strFileName,3,':') & "<br />" />
						<cfelse>
							<cfset variables.fullString = variables.fullString & "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" & "<br />" & ListGetAt(arguments.strFileName,3,':') & "<br />" />
						</cfif>
				...
				<cfcase value="cfm">
					...
					<cfloop list="#ListGetAt(arguments.strFileName,2,':')#" delimiters="," index="fileName">
						<cfsavecontent variable="variables.times">
							<cfsetting enablecfoutputonly="true">
							<cfloop from="1" to="6" index="i">
								<cfoutput>#dateFormat(Now(),"yyyymmdd")##timeFormat(Now(),"hhmm")##Chr(10)#</cfoutput>
							</cfloop>
