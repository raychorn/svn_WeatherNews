<cfoutput>
	<body onload="window_onload()" onunload="window_onUnload()">
		...
		<script language="javascript" type="text/javascript" src="/include/javascript/userLayerActions.js"></script>
		<script language="JavaScript1.2" type="text/javascript">
		<!--//
		...
		<!--
			...
			function handleMenuSelection(sObj, _spanName) {
				function sObj_text(_sObj) {
					return ((_sObj.selectedIndex == 0) ? '' : _sObj.options[_sObj.selectedIndex].text);
				};
				
				function sObj_value(_sObj) {
					return ((_sObj.selectedIndex == 0) ? '' : _sObj.options[_sObj.selectedIndex].value);
				};
				...
				var cached_sObj_text = sObj_text(sObj);
				
				if ( (!!dObj) && (!!tObj) ) {
					dObj.disabled = ((sObj.selectedIndex == 0) ? true : false);
					tObj.value = cached_sObj_text;
				}

				var brObj = menuEditorObj.brObj(global_menuEditorObj);
				if ( (!!brObj) && (sObj.selectedIndex > 0) && (global_menuEditorObj.isItemEditorOpen) ) { //  && (brObj.selectedIndex > -1)
					var mObj = getGUIObjectInstanceById('te_menuEditorPrompt');
					if (!!mObj) {
						mObj.value = cached_sObj_text;

						var sel = brObj.selectedIndex;
						if (sel > -1) {
							var _menuUUID = brObj.options[brObj.selectedIndex].value;
							var r = uMenuDict.getValueFor(_menuUUID);
	
							var aDict = DictionaryObj.getInstance(r);
							aDict.bool_returnArray = false;
						} else {
							var _menuUUID = uuid();
						}

				//	_alert('sObj.selectedIndex = [' + sObj.selectedIndex + ']' + ', brObj.selectedIndex = [' + brObj.selectedIndex + ']' + ', global_menuEditorObj.isItemEditorOpen = [' + global_menuEditorObj.isItemEditorOpen + ']' + ', cached_sObj_text = [' + cached_sObj_text + ']' + ', current_menu_selection_value_cache[_spanName] = [' + current_menu_selection_value_cache[_spanName] + ']' + '\n\naDict = [' + aDict + ']'); 

						global_dict.push(cached_sObj_text, [current_menu_selection_value_cache[_spanName],_spanName,_menuUUID]);
			...
			function processMenuNameEditAction(id, _spanName) {
				if ( (!!_spanName) && (typeof _spanName == const_string_symbol) ) {
					...
					if (!!tObj) {
						...
						//	_methodName = 'saveTopLevelMenuName'; // do nothing for this because there are no editing functions for layer data in menu editor...
						}
				...
				function displayRecord(_ri, _dict, _rowCntName, qryObj) {
					...
					if (true) {
						if (_ri == 1) {
							...
							_html += '<tr>';
							_html += '<td>';
							_html += '<select id="selection_' + _spanName + '" class="menuEditorDropDownClass" onchange="handleMenuSelection(this,\'' + _spanName + '\'); return true;">';
			...
			function _onCommitMenuMetaData(sPrompt, currentParentSubMenu_id) { // returns a Dictionary instance...
				...
				srcTableName = ((iAR.length > 1) ? iAR[1] : '');
				menuUUID = ((iAR.length > 2) ? iAR[2] : menuUUID);
							...
							if (!bool_isAnyErrorRecords) {
								...
								if (!!cObj) {
									...
									global_menuEditorObj.onOpenAddMenuItem = function (brObj) {
										...
										cObj = getGUIObjectInstanceById('selection_menuRecord');
										if (!!cObj) {
											cObj.disabled = true;
										}
										
										cObj = getGUIObjectInstanceById('div_contents_groupNameRecord');
										if (!!cObj) {
											cObj.disabled = true;
										}
	
										cObj = getGUIObjectInstanceById('selection_groupNameRecord');
										if (!!cObj) {
											cObj.disabled = true;
										}
									...
									global_menuEditorObj.onCloseMenuItemEditor = function (brObj) {
										...
										cObj = getGUIObjectInstanceById('div_contents_layerNameRecord');
										if (!!cObj) {
											cObj.disabled = false;
										}
									...
									global_menuEditorObj.onOpenAddSubMenu = function (brObj) {
										var cObj = getGUIObjectInstanceById('div_contents_menuRecord');
										if (!!cObj) {
											cObj.disabled = false;
										}
										...
										cObj = getGUIObjectInstanceById('selection_layerNameRecord');
										if (!!cObj) {
											cObj.disabled = true;
										}
										
										if (!!brObj) {
											brObj.selectedIndex = -1; // force the selection of a menu item to be ignored to force the new menu item to appear in the proper place in the menu...
										}
									...
									global_menuEditorObj.onSaveMenuItem = function (brObj, epObj) {
										...
										if (!!srcId) {
											...
											if (this.isAddingMenuSubMenu) {
												menuUUID = uuid(); // allocate a new uuid because we are adding a new menu item...
												dispOrder = (((!!brObj) && (brObj.selectedIndex > -1)) ? brObj.selectedIndex + 1 : brObj.options.length + 1);

												if (global_echo_AJAX_commands) _alert(111);
												doAJAX_func('onCommitSubMenu', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('srcTableName'), srcId, menuUUID, aDict.getValueFor('parentId'), dispOrder + 1);
												...
												var menuUUID = aDict.getValueFor('menuUUID');
												var kids = oMenuDict.getValueFor(menuUUID);

												if (this.isAddingMenuItem == false) {
													if (global_echo_AJAX_commands) _alert(222);
													doAJAX_func('onChangeSubMenu', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('srcTableName'), srcId, menuUUID, aDict.getValueFor('parentId'), dispOrder);
												} else {
													menuUUID = uuid(); // allocate a new uuid because we are adding a new menu item...
													dispOrder = (((!!brObj) && (brObj.selectedIndex > -1)) ? brObj.selectedIndex + 1 : brObj.options.length + 1);

													if (global_echo_AJAX_commands) _alert(333);
													doAJAX_func('onCommitMenuItem', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('srcTableName'), srcId, menuUUID, aDict.getValueFor('parentId'), dispOrder);
												}
...
// +++ Debug this function !
										function cutItemsToThinAir(_MENUUUID, _PARENTID) {
											...
											if ( (!!selected_container) && (selected_container.length > 0) ) {
												if (typeof selected_container != const_object_symbol) {
													var _ar = [];
													_ar.push(selected_container);
													selected_container = _ar;
												}
												if (selected_container != null) {
													oMenuDict.drop(_MENUUUID);
													...
													for (i = 0; i < selected_container.length; i++) {
														temp_Dict = DictionaryObj.getInstance(selected_container[i]);
														aMENUUUID = temp_Dict.getValueFor('MENUUUID');
														if (!!aMENUUUID) cutItemsToThinAir(aMENUUUID);
														DictionaryObj.removeInstance(temp_Dict.id);
													}
												}
											}
										...
										selected_MENUUUID = brObj.options[brObj.selectedIndex].value;
										if (typeof currentParentSubMenu_id != const_string_symbol) {
											currentParentSubMenu_id = currentParentSubMenu_id.toString();
										}
										currentParentSubMenu_id = ((currentParentSubMenu_id.trim().length == 0) ? '-1' : currentParentSubMenu_id);

										try {
											// BEGIN: Why are we bothering to do this now when all we need to do is let the Db perform this function and then display the dataset afterwards... ?!?
											cutItemsToThinAir(selected_MENUUUID, currentParentSubMenu_id);
											// END! Why are we bothering to do this now when all we need to do is let the Db perform this function and then display the dataset afterwards... ?!?
										} catch(e) {
											if (!!jsErrorExplainer) _alert(jsErrorExplainer(e, 'A. global_menuEditorObj.onDeleteMenuItem()' + ', currentParentSubMenu_id = [' + currentParentSubMenu_id + ']' + ', (typeof currentParentSubMenu_id) = [' + (typeof currentParentSubMenu_id) + ']' + ', selected_MENUUUID = [' + selected_MENUUUID + ']'));
										} finally {
										}
	
										if ( (!!oAR) && (oAR.length > 0) && (!!currentParentSubMenu_id) && (typeof currentParentSubMenu_id == const_string_symbol) ) {
											doAJAX_func('onDeleteMenuItem', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', currentParentSubMenu_id, oAR);
										} else if ( (currentParentSubMenu_id == null) || (typeof currentParentSubMenu_id != const_string_symbol) ) {
											alert('ERROR - Programming Error - The variable known as "currentParentSubMenu_id" has a value of (' + currentParentSubMenu_id + ') and this is not allowed.');
										}
