		...
		switch(functionToRun) {
			...
			case 12: 
				...
				if(SLO==1) { 
					...
					testArray = _level0.Layers.split(",");
					for(x=0;x<testArray.length;x++) {
						var tmpIDStore = getLayerIDFromName(testArray[x]);
						if(tmpIDStore > -1) {
							dumpArray[x] = tmpIDStore;
						}									
					}				
					//ShowHideLayersXML(dumpArray, 0);
					//_level0.debugBox.text+"bbox case12:"+BBox;
					//_level0.ToolBar.Refresh(); //Removed 12-6-2005
					break; 
				} 				
...
function PassCookieToCF() {	
	...
	//_level0.debugBox.text = _level0.debugBox.text+"PassCookieToCF:"+cookieOBJ.bBox;
...
function BboxCookieChange() {	
	...
	//_level0.debugBox.text = _level0.debugBox.text+"BboxCookieChange:"+bBoxCookieOBJ.bBox;
...
function HandOverBbox() {	
	...
	bBoxCookieOBJ.flashID = _level0.flashID;	
	bBoxCookieOBJ.bBox = _level0.WMSMap.BBox;
	//_level0.debugBox.text = _level0.debugBox.text+"HandOverBbox:"+bBoxCookieOBJ.bBox;
	bBoxCookieOBJ.StageW = Stage.width;
	bBoxCookieOBJ.StageH = Stage.height;
	bBoxCookieOBJ.mapLayers = _level0.IntStrFromCurrLyrCont();
...
function ShowHideLayersXML(tmpLayerList, tmpAction) {
				//_level0.debugBox.text = _level0.debugBox.text+"showhidelayersxml:"+tmpLayerList;
				...
				for(j=0;j<layerList.length;j++) {
					...
					else if((layerList[j] == 112)) {
						isPoly = 1;
						flagIT = 1;
					}
					else if((layerList[j] == 113)) {
						isCnvc = 1;
						flagIT = 1;
					}					
					...
					else if((layerList[j] == 143)) {
						isSigmet = 1;
						flagIT = 1;
					}
					else if((layerList[j] == 144)) {
						isAirmet = 1;
						flagIT = 1;
					}
				...
				if(isPoly == 1) {
					//_level0.Load_Poly("turb");
				}
				if(isCnvc == 1) {
					//_level0.Load_Poly("cnvc");
				}
				...
				if(isSigmet == 1) {
					_level0.Load_Poly("sigmet");
				}
				if(isAirmet == 1) {
					_level0.Load_Poly("airmet");
				}
				...
				if(tmpAction == 1) {
					_level0.ToolBar.SubmitLayers(0);  //what to do with this?	 -- REMOVED 12-5-2005
					toggleTools(false);  //Added 12-8-05
				}				
				/*
