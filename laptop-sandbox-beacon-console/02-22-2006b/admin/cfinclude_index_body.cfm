<cfoutput>
	<body onload="window_onload()" onunload="window_onUnload()">
		...
		<!--
			...
			//	_alert('sObj.selectedIndex = [' + sObj.selectedIndex + ']' + ', _spanName = [' + _spanName + ']' + ', dObj = [' + dObj + ']' + ', tObj = [' + tObj + ']');
			}
				...
				if ( (!!_spanName) && (typeof _spanName == const_string_symbol) ) {
					...
					if (!!tObj) {
						...
						if ( (_methodName.trim().length > 0) && (!!recID) ) {
							...
							alert('ERROR - Programming Error - Somebody forgot to code for the scenario in which the variable known as "recID" has this value (' + recID + ') in the function known as "processMenuNameEditAction()".');
						}
					//	perform_click_btn_dropMenuName(objID_prefix, iNum, recID, _spanName, _cfm_drop_cmd);
					//	_alert('processMenuNameEditAction(id = [' + id + '])' + ', _spanName = [' + _spanName + ']' + ', recID = [' + recID + ']' + ', tObj.value = [' + tObj.value + ']' + ', _methodName = [' + _methodName + ']');
					}
				}
			}
// +++
	...
	//	_alert('objID_prefix = [' + objID_prefix + ']' + ', iNum = [' + iNum + ']' + ', recID = [' + recID + ']' + ', _spanName = [' + _spanName + ']' + ', _cfm_drop_cmd = [' + _cfm_drop_cmd + ']');
				...
				function displayRecord(_ri, _dict, _rowCntName, qryObj) {
					...
					if (true) {
						...
						if (_ri == qryObj.recordCount()) {
							...
							_html += '</td>';
						//	_html += '<td>';
						//	_html += displayActionsPanel('span_' + _spanName + '_' + _col2_keyName + '_', _ri, _ID, 1, true, _col_keyNames);
						//	_html += '</td>';
							_html += '<td>';
					...
					//	_alert('_ri = [' + _ri + ']' + ', qryObj.recordCount() = [' + qryObj.recordCount() + ']');
					} else {
...
//_alert('\nqData = [' + qData + ']');
			...
			function handleLayerEditorMetaData(qObj) {
				...
				function aKeyWithGreatestValue(_k, _v) {
					...
					_greatestValue = ((bool) ? x : _greatestValue);
				//	if (bool) {
				//		_alert('aKeyWithGreatestValue(_k = [' + _k + '], _v = [' + _v + '])');
				//	}
					return bool;
				};
...
//_alert('vTotal = [' + vTotal + ']' + ', vDiff = [' + vDiff + ']');
//_alert('\naa = [' + aa + ']' + ', (aa.pop()) = [' + aa.pop() + ']');
//_alert('\nglobal_LayerEditorMetaData = [' + global_LayerEditorMetaData + ']');
//_alert('\nglobal_EditableFieldMaxLengths = [' + global_EditableFieldMaxLengths + ']');
//_alert('\nglobal_EditableFieldSizes = [' + global_EditableFieldSizes + ']');
						}
					}
				}
				DictionaryObj.removeInstance(argsDict.id);
