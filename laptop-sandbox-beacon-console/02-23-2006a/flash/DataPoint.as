//Primitive.as
	...
	class DataPoint extends MovieClip {
		...
		public function addPoint (tmplat,tmplon):Void {		
			//_level0.debugBox.text = _level0.debugBox.text+tmplat+","+tmplon+"<br>";
			DP_Lat = tmplat;
			DP_Lon = tmplon;
  		}
		...
		public function drawPoint():Void {	
			...
			if(DP_Ctemp != undefined && DP_PTType == 3) {					
				...
				if(DP_PTType == 8) {	
					...
					else if(crossDist < 500 && crossDist > 200) {
						DP_Size = 30;
					}
					else if(crossDist < 1000 && crossDist > 500) {
						DP_Size = 20;
					}
					else if(crossDist > 1000 && crossDist < 2000) {
						DP_Size = 20;
					}
					else if(crossDist > 2000) {
						DP_Size = 20;
					}
			...
			//if(_level0.WMSMap[testClipName]._x < 0) { _level0.WMSMap[testClipName]._x = Math.abs(_level0.WMSMap[testClipName]._x); } //changed 12-13-05
		...
		public function killPoint():Void {
			//_level0.debugBox.text = _level0.debugBox.text+"kill point called:"+DP_ClipName;
		...
		private function parseBbox(tmpX, tmpY):Array {
			...
			var XMax = Number(parsebox[2]);
			var YMax = Number(parsebox[3]);
			if(tmpX < 0 && parsebox[0] > 0) { tmpX = tmpX+360; }
			delete parsebox;			
			//then you calculate the map coordinate to pixel ratio:
			//_level0.debugBox.text = _level0.debugBox.text+"TMPX:"+tmpX+"  TMPY:"+tmpY+"<br>";
			//_level0.debugBox.text = _level0.debugBox.text+"XMIN:"+XMin+"  YMAX:"+YMax+"<br>";
			var xscale = _level0.WMSMap.Width/(XMax-XMin);
			//_level0.debugBox.text = _level0.debugBox.text+"XSCALE:"+xscale +"<br>";
			var yscale = _level0.WMSMap.Height/(YMax-YMin);	
			//_level0.debugBox.text = _level0.debugBox.text+"YSCALE:"+yscale+"<br>";
			
			parseBoxArray[0] = (tmpX-XMin)*xscale;
			parseBoxArray[1] = (YMax-tmpY)*yscale;
			//_level0.debugBox.text = _level0.debugBox.text+"PB:"+parseBoxArray[0]+","+parseBoxArray[1]+"<br>";
