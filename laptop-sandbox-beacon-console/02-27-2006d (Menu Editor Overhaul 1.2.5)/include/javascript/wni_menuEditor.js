				...
				for (i = 0; i < iAR.length; i++) {
					...
					if (hasChildren) {
						...
						for (j = 0; j < oChildren.length; j++) {
							...
							_html += '<input type="' + ((bool_useRadioButtons == false) ? 'Checkbox' : 'Radio') + '" class="sideMenuCB" id="cb_menuItem_' + mUUID3 + '"' + ((bool_useRadioButtons == false) ? '' : 'name=rb_menuItem_' + this.aSUBMENUUUID) + ' onclick="ClientMenuObj.onCheckBoxClicked(ClientMenuObj.instances[' + this.id + '], this.id, this.checked); return true;">&nbsp;' + '<span class="menuClass">' + tDict3.getValueFor('PROMPT') + '</span>';
							_html += '</td>';
							_html += '</tr>';
						...
						if (bool_useLinks == false) {
							_html += ((mUUID.trim().length > 0) ? '<input type="' + ((bool_useRadioButtons == false) ? 'Checkbox' : 'Radio') + '" class="sideMenuCB" id="cb_menuItem_' + mUUID + '"' + ((bool_useRadioButtons == false) ? '' : 'name=rb_menuItem_' + this.aSUBMENUUUID) + ' onclick="ClientMenuObj.onCheckBoxClicked(ClientMenuObj.instances[' + this.id + '], this.id, this.checked); return true;">&nbsp;' : '');
						}
