<cfcomponent hint="qryGetPIREPS">
	<cffunction name="init" access="public" returntype="query">
		...
		<cfargument name="strStationList" type="string" required="no" />
		...
		<!--- region --->
		<cfargument name="region" type="string" required="no" default="USA" />
		<cfif IsDefined("arguments.strStationList")>
		...
		<cfelse>
			<cfset qryPIREPS = getPIREPS(arguments.intPeriodFrom,arguments.intPeriodTo,
										   arguments.strIntensityList,
										   arguments.pType,
										   arguments.pTypeOther,arguments.region) />
		</cfif>
	...
	<cffunction name="getPIREPS" access="private" returntype="query">
		...
		<cfquery name="getPIREPS" datasource="#AVNOPS_DS#">
					SELECT 
							Col_1=P.origin, 
