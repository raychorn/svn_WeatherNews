<cfcomponent displayname="layerDataGateway" output="no">
	<cfset variables.debug_file_path = "#Request.webRoot#include\debug\">
 ...
 --->	
	...
	<cffunction name="parseCellTopsMotion" access="remote" returntype="array" output="false">		
		...
		//currentZoom = 40;
		 ...
		 <cffile action="read" file="#Request.webRoot#flash\xml\conus_topsmotion.xml" variable="ctmFile"> 
		<!---<cffile action="read" file="#Request.webRoot#flash\xml\newtops.xml" variable="ctmFile">--->
	...
	<cffunction name="parse00ZFntHiLo" access="remote" returntype="struct" output="false">
		...
		<cffile action="read" file="#Request.webRoot#flash\xml\fronts_00.xml" variable="front00File">		
		...
		<cffile action="read" file="#Request.webRoot#flash\xml\hilo_00.xml" variable="hilo00File">		
	...
	<cfscript>
		function funcBuildSimpleContours(sArgs, xmlObj) {
			var i = -1;
			var p = -1;
			...
			for(i=1;i LTE ArrayLen(selectedElements); i=i+1) {  //Changed the i to equal 1 instead of two.
				...
				for(p=1;p LT ArrayLen(tmpLineList);p=p+2) {
					if((tmpLineList[p] GT sArgs.struct_coordinates.x3 AND tmpLineList[p] LT sArgs.struct_coordinates.x2) AND (tmpLineList[p+1] GT sArgs.struct_coordinates.y3 AND tmpLineList[p+1] LT sArgs.struct_coordinates.y1)) {
						succCnt = 1;		
					}					
				}
				if(succCnt EQ 1) {			
					sVals.eleArray[i+errorCnt] = dataStruct;		
				}
				else {
					errorCnt = errorCnt + 1;
				}
	...
	<cffunction name="buildSimpleContours" access="public" returntype="array" output="false">	
		...
		<cfif (FindNoCase(".wni.com", CGI.SERVER_NAME) eq 0)>
			<cfscript>
				sArgs.struct_coordinates = struct_coordinates;
				sRet = Request.xmlCacheObj.xmlCacheRead(#Request.webRoot#flash\xml\#tmpFName#, funcBuildSimpleContours, sArgs);
				return sRet.eleArray;
			</cfscript>
		<cfelse>
			<cffile action="read" file="#Request.webRoot#flash\xml\#tmpFName#" variable="tmpFilePoint">
