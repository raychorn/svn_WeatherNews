	...
	<cfif ListFindNoCase(Form.Fieldnames, "ACCOUNTSUBMIT")>
		...
		<cfif Len(gErrString) EQ 0>
			...
			<cfelse>
				...
				<cfquery NAME="uUserInformation" DATASOURCE="#Request.INTRANET_DS#">
					Update AvnUsers
	...
	<cfelseif ListFindNoCase(Form.Fieldnames, "PASSSUBMIT")>
		...
		<cfif Len(gErrString) EQ 0>
			...
			<cfelse>
				<cfquery NAME="getUserStatsFromLoginID" DATASOURCE="#Request.INTRANET_DS#">
	...
	<cfelseif ListFindNoCase(Form.Fieldnames, "IPSubmit") OR ListFindNoCase(Form.Fieldnames, "SATSubmit") OR ListFindNoCase(Form.Fieldnames, "RADSubmit")>
		<cfif ListFindNoCase(Form.Fieldnames, "IPCHECKBOX")>
			<cfquery NAME="uFlightInformation" DATASOURCE="#Request.INTRANET_DS#">
				UPDATE AvnFlights
				SET Deleted = 1, 
					SetAsDefault = 0
		...
		<cfif ListFindNoCase(Form.Fieldnames, "IPRADIOBUTTON")>
			<cfquery NAME="uFlightInformation1" DATASOURCE="#Request.INTRANET_DS#">
				UPDATE AvnFlights
				SET SetAsDefault = 0
				FROM AvnUsers U, AvnFlights F	
				WHERE U.LoginID = '#ListGetAt(cookie.BeaconStats,1,"|")#' AND U.UserID = F.UserID
			...
			<cfquery NAME="uFlightInformation2" DATASOURCE="#Request.INTRANET_DS#">
				UPDATE AvnFlights
				SET SetAsDefault = 1
				FROM AvnUsers U, AvnFlights F	
				WHERE U.LoginID = '#ListGetAt(cookie.BeaconStats,1,"|")#' AND U.UserID = F.UserID
