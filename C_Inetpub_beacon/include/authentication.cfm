...
<cfif (not isdefined("cookie.initialized")) OR (bool_override_logic)>
	...
	<cfif not isdefined("cookie.rememberMe")>
		<cfif not isdefined("uname") or not isdefined("pword")>
			<cflocation url="/#Request.const_login_htm#?ghjghdasd">
			<cfexit>
		</cfif>
	<cfelse>
		...
		<cfelse>
			<cfif (IsDefined("cookie.BeaconStats"))>
				...
				<cfif ( (IsDefined("uname")) AND (IsDefined("pword")) )>
					<cfset ss = doesLoginID_matchThisValue(Request.LoginID, uname, pword)>
...
<cfelse>
	<cfif not isdefined("cookie.initialized") or not isdefined("cookie.rememberMe") or not isdefined("cookie.BeaconStats") >
		<cflocation url="#Request.const_login_htm#?logout">
	</cfif>
	
	<cfoutput>
	<cfif (DateDiff("h",ListGetAt(cookie.BeaconStats,4,"|"),now()) GT 48)>
		<cflocation url="#Request.const_login_htm#?logout&reason=1">
	<cfelse>
